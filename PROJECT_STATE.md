...
当前状态：架构和工程协议已完成评审，六个架构探针的外部事实已完成（PROBE-001~004 为 `PASS`，PROBE-005~006 为 `PASS_WITH_ADJUSTMENT`）；T0 Monorepo 与本地开发基线已真实环境验收通过（2026-08-28）；**T1a Manifest/Prisma Core 切片 + devex P1 三项已于 2026-09-01 获用户 `ACCEPTED_WITH_ACTIONS`**（条件见[验收记录 §6](docs/engineering/acceptance/hg-01-t1a-manifest-core.md#6-用户验收结论)：租户谓词与请求体 `tenantId` 退场归 T14 DoD，幂等重放 201→200 归 T2）。HG-01 门禁本身尚未通过——完整范围还差 T12a(Ledger/配置骨架)、T11a(同步审计骨架)、T14a/T14b(身份与授权)（前置文档已于 2026-09-03 补齐：ADR-0040 与 T11 票据、T14 批次划分、HG-01a 临时门禁登记、`AGENTS.md` 与 T11/T12 的三份冻结契约，见[下一步执行顺序](#下一步执行顺序)第 4 条）。**T12a 三片已于 2026-09-04 全部落地并自测通过（价目与配额配置、两表及迁移、五条事务入口 + `writeAuditEvent` + 原因码注册表 + 两层测试），但库层入口尚无调用方——模型调用侧归 T15，所以运行时仍然没有任何请求在预扣、结算或写审计**；**T14a 身份接入于 2026-09-07 落地、2026-09-08 通过 HG-01a 人工验收（`ACCEPTED`，[验收记录](docs/engineering/acceptance/hg-01a-t14a-identity-foundation.md)）；T14b 统一授权入口已于 2026-09-08 落地自测通过（授权契约与 authz 审计码、aclRevision 列、能力解析/作用域编译/候选复核查库入口、统一授权入口含决策审计与 fail closed、tenantId 退场与租户谓词、撤权/跨租户/复核集成测试、smoke 全绿），同日代码评审发现的缺口已修复（数据等级拒绝补齐、作用域键统一走 contracts 编码器、四类 approve*/Release 的跨租户真库测试补齐，详见「已完成内容」），待用户复核**；T2/T10/T3/T1b 的解锁待完整 HG-01 核验。除 T1a 的 Manifest/Release 领域面外尚无业务实现；容器级集成测试（Testcontainers）、Playwright、部署和真实语料性能基线均未验证。探针结果索引、Probe Decision Gate 和 ADR-0037 记录了已冻结事实、集成测试门槛与生产治理遗留项。
...
- 领域审计同步写 PostgreSQL；Trace、Token、成本和高频遥测异步投递，不让遥测系统成为业务提交依赖；`traceId/spanId` 生成内建，不依赖常驻观测栈。两条载体按 [ADR-0040](docs/adr/0040-domain-audit-and-runtime-telemetry.md) 分在不同包上：审计写入口只在 `packages/database` 且必须传入已开启的事务，`@rag/observability` 不得导出审计入口；原因码集中登记在 `packages/contracts/src/audit/`。
...
- 已完成 **T14b 统一授权入口与资源策略**（2026-09-08；同日代码评审后补齐与修复）：`@rag/contracts` 授权契约（`AuthorizationDecision` 四类拒绝原因、`acl_scope_key` 定形 `t:{tenantId}:ks:{ksId}`、authz.* 审计码五枚入注册表）；`tenants.aclRevision` 列与 `bumpAclRevision`（成员变更必须同事务递增，ADR-0026）；`packages/database` 授权查库四入口（能力解析两层并集、作用域编译集合与 revision 同次读取、候选复核批量减法）；`apps/api` 全局 `AuthorizationService`（能力先于资源、每次判定写同步领域审计、允许路径审计写失败不放行、依赖不可用 fail closed）；**T14 DoD 两条退场已执行**——manifests 五 schema 移除请求体 `tenantId`（测试钉住「携带不生效」）、`approve*`/`GET /releases/:id` 全部带租户谓词（跨租户 id → 404 NOT_FOUND 不是 403，`tests/manifests-tenant-isolation.test.ts` 在真 PostgreSQL 钉住四类 approve 与 Release 读取）；IdentityGuard 从会话推导单一 ACTIVE 租户（多个 fail closed，多租户切换器归 T16）。smoke-api.sh 改走完整 OIDC（`/auth/login` → 表单登录 → 回调带完整 query 含 state，登录块抽为 `oidc_login()` 函数）。**评审补齐（数据等级拒绝）**：资源策略层对 `document_version` 的 UNKNOWN/SENSITIVE 拒绝（阶段 1 无 clearance 模型的 fail-closed 默认；`DATA_CLASS_DENIED` + `authz.dataclass_denied`），候选复核只随行 `dataClasses` 元数据、不做等级减法——敏感候选的执行区阻断在 T15 准入层（ADR-0025），两层职责不得互换；`documentVersionDataClasses` 契约与 Prisma 枚举漂移由 schema 文本断言钉住。**评审修复**：作用域键统一走 contracts 的 `scopeKeyForKnowledgeSpace`（删除手拼与两臂相同的三元死代码）、`AuthzReasonCode` 收窄类型收敛服务层两处联合、`compileAllowedScopes` 删除产不出的 `USER_NOT_FOUND` 并加类型级子集断言防口径漂移、ADR-0039 决策引用修正。复核超时接线（T6）、多角色浏览器 E2E（T16/HG-01）、统一入口调用点（T6）三项推迟已登记进[票据批次划分](docs/engineering/tickets/T14-identity-authorization.md#批次划分)。
- 已完成 **T14a 身份接入与业务身份模型**（2026-09-07；2026-09-08 完成代码审查修复）：ADR-0039 的 7 张业务身份表、初始迁移（`20260907063419_t14a_business_identity`）及角色边界加固迁移（`20260908090000_t14a_role_scope_guards`，成员角色复合租户外键 + RoleScope 触发器）、`packages/database` 的 `loadIdentityContext` 装配入口（数据库包持有事务边界；判别联合区分未建档/禁用，不 JIT、不过滤成员状态）、`packages/contracts/src/auth` 的服务端身份上下文与 SessionView 投影（不外发 issuer/subject）、`apps/api/src/auth` 的 OIDC 授权码 + PKCE + JWKS 会话（`/auth/login|callback|session|logout`；会话过期 401、Keycloak 网络故障及 5xx 映射 503、`AUTH_REQUEST_TIMEOUT_MS` 同时约束 token/JWKS 请求）、进程级随机本地会话密钥（生产缺失时 fail-fast）、开发种子（三 Workspace 多角色，issuer/subject 可随 Keycloak 配置自愈）与 `init-keycloak.sh` 的 partialImport 固定 UUID 用户 + `rag-api` 客户端。Keycloak 七类场景（PKCE、JWKS 轮换、过期、禁用、撤权、不可用、恢复）分两层：五类真实容器集成（`tests/keycloak-oidc.test.ts`，本地跑）、两类单元（fetch 桩）；角色跨租户和 scope 约束另由 `tests/identity-role-scope.test.ts` 在真实 PostgreSQL 验证。**HG-01a 人工验收已于 2026-09-08 通过（`ACCEPTED`，无行动项）**：用户在真实浏览器走通登录链路、核验 logout 401 信封、表结构与守卫触发器；[验收记录](docs/engineering/acceptance/hg-01a-t14a-identity-foundation.md)。授权判定与 `tenantId` 退场归 T14b。
...
- 每日 16 元、用户级配额和 24 至 36 周窗口仍需在真实链路下校准；票据级工作量估算已补齐，但周期换算系数尚未用真实交付速率验证。完整 24 GiB Parser 余量也尚未验证。ADR-0034 的五个配额值与 PROBE-005 反算的单价、汇率 7.2 已于 2026-09-03 以启动期配置默认值落地（`packages/config/src/resource-limits.ts`、`packages/config/src/model-pricing.ts`），但**没有任何运行时路径读它们**：限流、预扣与结算都还不存在，配置落地不等于门禁生效。
...
4. **进行中**：按[阶段 1 实施 Tickets](docs/engineering/stage1-implementation-tickets.md)推进 T1a + [T12a Ledger 与配置骨架](docs/engineering/tickets/T12-performance-budget.md#批次划分) + [T11a 同步审计骨架](docs/engineering/tickets/T11-audit-telemetry.md#批次划分) + [T14a/T14b 身份与授权](docs/engineering/tickets/T14-identity-authorization.md#批次划分)。**T1a 切片与并入的 devex P1 三项（DX-T1/DX-T2/DX-T3）已提交为 `bc99b0a` + `1b2a2ed`，并于 2026-09-01 获用户 `ACCEPTED_WITH_ACTIONS`**（[验收记录 §6](docs/engineering/acceptance/hg-01-t1a-manifest-core.md#6-用户验收结论)：租户谓词与请求体 `tenantId` 退场归 [T14 DoD](docs/engineering/tickets/T14-identity-authorization.md#dod)，幂等重放 201→200 归 T2）。该结论只覆盖 T1a 切片，**不等于 HG-01 门禁通过**，也不授权 commit/push/PR/merge/部署。
   同批次剩余按依赖顺序推进：**T12a Ledger/配置骨架 → T11a 同步审计骨架 → T14a（Keycloak auth + 7 张身份表与迁移）→ T14b（统一授权入口、能力权限、资源策略、`acl_scope_key` 编译、fail closed）**；T11a 必须先于 T14b，因为 T14 DoD 要求授权决策写同步领域审计，原因码要落在 T11a 的中央注册表。三张票据的前置文档已于 2026-09-03 补齐：[ADR-0040](docs/adr/0040-domain-audit-and-runtime-telemetry.md) 定义审计/遥测的载体与原因码口径，[T11 票据](docs/engineering/tickets/T11-audit-telemetry.md)与 T14 的 `## 批次划分` 落地批次边界。同日进一步补齐四件交接前置（`packages/database/src` 只有 client/index/env/health，没有 repository 或 service 层先例，签名不定死会各拍一个形状）：仓库根新增 [`AGENTS.md`](AGENTS.md) 作为 Agent 唯一入口；[T12 事务入口契约](docs/engineering/tickets/T12-performance-budget.md#事务入口契约)定死五条预算入口签名、金额用 `Prisma.Decimal`、超限返回判别联合而不抛异常，并给四类预算审计定名与 `outcome` 映射；[T11 审计写入口契约](docs/engineering/tickets/T11-audit-telemetry.md#审计写入口契约)定死 `writeAuditEvent(tx, event)` 与原因码注册表形状（`category` 由注册表派生，调用方不传）；[T12 预扣估值价格表与汇率](docs/engineering/tickets/T12-performance-budget.md#预扣估值价格表与汇率)从 PROBE-005 的 LIVE 报文按供应商返回的 `usage.cost` 反算冻结单价与汇率 7.2，rerank 估值给出「候选数 × 108 token」公式并要求 ±5% 内复现四个 LIVE 锚点（全表标注初始值可校准，校准不重开 ADR）。T12a 的账本 schema 与事务入口可以先落，但它的四类审计写入必须与 T11a 同批合并、迁移合到同一次评审。T14 按 [ADR-0039](docs/adr/0039-business-identity-and-unified-authorization.md) 实现，票据要求身份迁移与授权代码分开提交、分开评审，故 T14a 结束时增设一个临时人工门禁，已登记为门禁表中的 [HG-01a（临时）](docs/engineering/manual-acceptance-gate.md#阶段-1-门禁点)。四项全部完成后才是完整 HG-01 核验，通过后方可启动 T2/T10/T3/T1b。
   **T12a 进展（2026-09-03，进行中）**：按票据的[拆分依据](docs/engineering/tickets/T12-performance-budget.md#工作量估算)分三片交付，每片一个 PR。第一片「配置骨架」已完成：`packages/config/src/resource-limits.ts` 补池三分 350/100/50、lease 默认 60s 与续租总时长上限、ADR-0034 五项用户级配额与管理侧 `rebuild` 并发；新增 `packages/config/src/model-pricing.ts` 承载单价、汇率 7.2 与四条估值函数（Embedding / Rerank / Chat / 一次问答合计）。两类值性质不同，已分别由测试钉住：5/16/500 与池三分是代码级上界（配出更大即启动失败），ADR-0034 那五个只是默认值、允许运行配置上调（该 ADR 明确校准不重开 ADR），`.max()` 只防数量级手滑。rerank 估值在 ±5% 内复现四个 LIVE 锚点（候选 8/64/256/1024 → ¥0.0012/0.0099/0.0397/0.1587），Chat 估值复现 PROBE-005 单次问答 ¥0.00064；缓存命中价默认等于未缓存价，因为没有任何实测或公开价目支撑折扣，编一个折扣率只会让预扣系统性偏低。自测：`pnpm run verify` 全绿（18 文件 / 159 测试，较 T1a 的 131 增 28），`packages/config/src` 覆盖率 98.41 / 97.61 / 100 / 98.41，整体四项 88.05 / 82.22 / 85.34 / 88.83 仍在阈值 86/81/82/87 之上。剩余两片：② `model_budget_ledger` 与 `domain_audit_event` 合到同一次迁移评审（票据要求两表同批，且迁移单独提交）；③ `packages/database/src/budget/` 五条事务入口 + `packages/database/src/audit/` 的 `writeAuditEvent` + `packages/contracts/src/audit/reason-codes.ts` 注册表，配集成测试（并发预扣竞态、幂等重放、lease 回收、审计写失败回滚业务事务）。
   **T12a 第二片（2026-09-03）**：`model_budget_ledger`（ADR-0029）与 `domain_audit_event`（ADR-0040）合到同一次迁移 `20260903132723_t12a_budget_ledger_and_domain_audit`，依据是[T12 票据第 212 行](docs/engineering/tickets/T12-performance-budget.md#依赖与时点)与[T11 票据第 147 行](docs/engineering/tickets/T11-audit-telemetry.md#依赖与时点)要求两表同批评审；按 T1a 口径迁移单独提交，不与门禁代码混在一起。三处刻意的取舍写进 schema 注释，防止下一个会话「补回来」：**没有 `period` 列**（一行预扣同时算进当日与当月两个窗口，单值填不了，一行一窗口又会让账本翻倍，所以两个窗口按 `createdAt` 范围 CAS，预算日时区归 T12b 运行配置）、**终态时间戳只有 `finalizedAt`**（RESERVED 之后恰好一次终态转移）、**`answerRunId`/`jobId` 暂无外键**（AnswerRun 归 T7、job 归 T3，恰好一个非空由 CHECK 保证）。Prisma 表达不了的库层不变量用手写 SQL 补，口径照 `20260828130000_manifest_immutability`：金额非负、汇率为正、`status` 与终态字段一致、终态行与预扣事实由 BEFORE UPDATE 触发器锁死、`reasonCode` 必须是 `域.动作` 命名空间格式、审计行行级 UPDATE/DELETE 一律拒绝（TRUNCATE 不触发行级触发器，集成测试清理照旧可用）。自测三步：三条迁移重放进空库后 `migrate diff` 对 schema.prisma 为空、12 条库层不变量逐条冒烟（各自拒绝了它声称要拒绝的写入，合法的预扣→续租→结算通过）、`migrate deploy` 应用到开发库后 `migrate status` 报 up to date。顺带修掉一处 T1a 遗留漂移：七个复合外键的约束名在迁移里是手写的，schema 不声明 `@relation(map:)` 时 Prisma 推导出另一套名字，于是每次 `prisma migrate dev` 都判定开发库漂移并要求 reset（修在 `98f9c8d`，纯口径修正、不改表结构）。本机开发库另需两次一次性修复（清 6 个只存在于本机的旧 `*_tenantId_idx`、修 t1a 迁移的 `_prisma_migrations.checksum`），**新克隆不需要**——空库重放三条迁移后与 schema.prisma diff 为空即证。第三片（五条事务入口 + `writeAuditEvent` + 原因码注册表 + 集成测试）未开始。
   **T12a 第三片（2026-09-04）**：五条事务入口（`reserveBudget`/`settleBudget`/`releaseBudget`/`expireBudgetLeases`/`renewBudgetLease`，落 `packages/database/src/budget/`）、审计写入口（`writeAuditEvent`，落 `packages/database/src/audit/`）与原因码注册表（`packages/contracts/src/audit/reason-codes.ts`，四个 `budget.*` 码，`category` 由注册表派生、调用方不传）全部按[事务入口契约](docs/engineering/tickets/T12-performance-budget.md#事务入口契约)与[审计写入口契约](docs/engineering/tickets/T11-audit-telemetry.md#审计写入口契约)的签名落地：超限返回判别联合而不抛异常，只有非法转移与读不到账本才是异常；审计写失败一律抛，让外层事务连业务写入一起回滚（与遥测入口最本质的区别）。包根只导出这五个函数、`writeAuditEvent` 与它们的类型，`sql.ts`/`windows.ts`/池上限查表不出包，T15 拿不到 `PrismaClient` 也拿不到表结构。**测试分两层，分界按「证据在哪里」而不是按快慢**：假事务能证明调用顺序与入参校验，证不了两个连接同时打进来时数据库怎么排队，所以 T12「验证」小节点名的四条（并发预扣撞日限只有一个成功、幂等重放不重复扣款、lease 回收把 `RESERVED` 变 `EXPIRED` 并释放额度、租户 A 不影响租户 B）连同库层触发器与 CHECK 全部落进新的 `tests/` 集成层，跑真 PostgreSQL；新增 `vitest.integration.config.ts` 与 `pnpm run test:integration`，并接进 `integration.yml` 的 `smoke` job（放在 `smoke:api` 之前：只依赖 PostgreSQL，失败面更窄）。自测：单元层 23 文件 / 262 测试全绿（较第一片的 18/159 增 5 文件 103 测试），集成层 3 文件 / 27 测试全绿并连跑 20 轮零偶发；覆盖率四项 90.56 / 86.28 / 87.85 / 91.18，棘轮随之由 86/81/82/87 抬到 89/85/86/90（不抬的话以后删掉这一片一半的用例仍然能过）。**集成层照出三个假事务照不出的库层缺陷，三个都已修**：① `pg_advisory_xact_lock` 返回 `void`，Prisma 驱动反序列化不了，于是每一次真库预扣都死在第一条语句上——改成 `::text AS "locked"` 取一个可反序列化的值，且仍留在 `$queryRaw` 里，单元层「锁在幂等查询之前」的调用顺序断言照旧成立；② Prisma Client 为 `@default(now())` 在**客户端**生成值，列的 `DEFAULT CURRENT_TIMESTAMP` 只对省略该列的写入者（裸 SQL）生效——实测同一事务里两次 `create()` 差几毫秒，于是四层 CAS 按事务时钟校验窗口、行却落在应用时钟上，跨零点的那一次调用正好绕过日限；修法是新增内部 `src/tx-clock.ts`（不进 `index.ts`，审计与账本共用同一口时钟，且 `audit/` 不必 import `budget/`），`createdAt` 与 `occurredAt` 由入口显式传 `txNow(tx)`，schema 保持 `@default(now())` 因此 `migrate diff` 仍为空、无需新迁移，列默认值留作裸 SQL 的兜底；被否掉的另一种修法是 `@default(dbgenerated("CURRENT_TIMESTAMP"))`，它会造成永久且无法消解的 `migrate diff` 漂移（库把 `CURRENT_TIMESTAMP` 反查成 `Now`）。这条修完顺带把不变量搬进了单元层：假事务在调用方省略时间列时刻意填一个差 5 秒的「客户端时钟」，于是「入口是否显式传了库时钟」在快层就能断言；③ `RETURNING` 的行序是执行计划的副产物，PostgreSQL 不保证它等于子查询的 `ORDER BY`——回收批次实测出现两行反序（约 3~8 轮一次），而返回值要逐条写审计，行序不定会让「回收顺序」成为计划的函数；修法是把候选 `SELECT` 与 `UPDATE` 收进 CTE，在外层重新 `ORDER BY "leaseExpiresAt"`，多排最多 `limit` 行换确定的批次。集成层的清理是**不对称**的，这是不变量的直接结果而不是偷懒：账本行必须删（回收任务不带 `tenantId`，残留 `RESERVED` 会混进下一轮批次；状态机触发器是 `BEFORE UPDATE`，不管 DELETE），审计行删不掉（`domain_audit_event_append_only` 对行级 UPDATE/DELETE 抛 `check_violation`），**写过审计的**租户跟着删不掉（审计外键 `ON DELETE RESTRICT`），其余租户照删——一次运行里绝大多数租户只做预扣与回收、没有审计行，全留下的话每跑一次开发库就多积几十个死租户（实测二十来轮攒下 1096 个）；于是残留收敛成每轮少数几个 `integration-*` 租户和它们的审计行，只能靠 `pnpm run infra:reset` 清，CI 每次是全新容器不受影响；测试路径刻意不用 TRUNCATE 这条后门（当时迁移里留着它，见下一段：那条后门已在外部评审后堵掉）。给根 `package.json` 加上 `@rag/config`/`@rag/database` 这一步还照出一个 T0 起就在的 bootstrap 缺陷：`init-database.sh` 的 seed 前置构建把 pnpm 的 `--filter "...@rag/database"` 写成了**前置** `...`（依赖方）而不是后置（依赖），一直靠 api/worker 的 `tsc -b` 顺带构出 `packages/*/dist` 才没露馅；根成了依赖方之后，它的递归 build 把 `apps/web` 的 `next build` 拉进了 bootstrap，在 `NODE_ENV=development` 下预渲染 `/_global-error` 崩掉（Next 不支持以 development 构建），改成 `@rag/database...` 后 scope 收敛成 database + config + contracts + observability 四个（详见 CHANGELOG）。这一片让 T12a 的库层不变量可证，但**门禁尚未端到端生效**：五条入口还没有调用方，真实请求的预扣/结算/写审计要等 T15 接线。
   **T12a 外部评审修正（2026-09-04）**：拿 Codex 对 `main...c214812` 做了一轮独立评审，结论是"可作为配置 + 表结构切片验收，不可作为 T12a/T11a 完成验收"，0 条 AGENTS.md 违规。它列为阻塞的"五条事务入口与审计写入口缺失"是**范围假象**——`c214812` 是第二片 PR 的头，第三片的代码在下一个 PR 的 `78d8b1b` 里，不在被评审的 diff 内。两条 P1 是真的，都已修（详见 CHANGELOG）：① **单价 0 能通过 schema**，`z.coerce.number().nonnegative()` 配上 `z.coerce.number()` 把 `''`/`null`/`' '`/`[]`/`false` 全折成 0（zod 4.5.4 实测），而 T12b 正要从环境变量读这几个值；单价 0 让那条腿的预扣恒为 0，四层 CAS 全过、门禁静默打开。改 `.positive()`（与 `resource-limits.ts` 每个数值字段同一口径，`.nonnegative()` 原是全包唯一例外），并给四个估值入口补有限非负整数校验——`estimateRerankCny(..., { candidateCount: -1 })` 实测返回 `-0.000156`，而 `estimateAnswerRunCny` 的 `verification.*` 必须**四项各自校验**，只校验乘积挡不住 `-1 × -1 = 1`。NaN 比负数更危险：负数在四条腿求和里只是抵消、把总额抹低，NaN 却让账本里每个 `greaterThan` 都成假。用例 37 → 58 条。② **两张事实表可被一条语句清空**：append-only 触发器是 `FOR EACH ROW`，TRUNCATE 不触发行级触发器，上一片迁移的注释还把这条路当成"清库路径仍然通畅"记了下来；审计表因此在一条语句面前不是不可变的，账本更糟（余额事实源只有它，清空等于把花销归零而 Redis 还缓着旧值）。新迁移 `20260904133000_audit_and_ledger_truncate_guard` 给两张表各加 `BEFORE TRUNCATE ... FOR EACH STATEMENT` 触发器抛 `check_violation`（语句级是唯一对 TRUNCATE 触发的形式），集成层两条裸 SQL 用例撞它、含 `TRUNCATE "tenants" CASCADE`（级联表上同样触发，实测通过），用例包在会回滚的事务里，防线万一没了是测试红而不是先清表再报红。**边界写在迁移注释里，不留错觉**：表 owner 仍可 `DROP TRIGGER` 再 TRUNCATE，这道防线把"手抖清库"变成响亮报错、把绕过变成 review 看得见的显式 DDL，真正的硬边界是应用角色与迁移角色分离（T14/部署期）。另一条 P2 是文档缺口：`resource-limits.ts` 六个 `.max()` 只活在代码注释里，已补进 [ADR-0034](docs/adr/0034-per-user-rate-limit-and-concurrency-quota.md) 的「2026-09-04 补充：配置 schema 的安全上界」，含"为什么这些上界挡不住任何真实校准值"的反算（每日 16 元在 1024 候选下约 100 次问答、64 候选下约 1600 次，先耗尽的一定是预算），**六个默认值未变**。
   **前置待用户动作**：远端已换成 SSH（`git@github.com:Axwen/myRAG.git`，仓库已于 2026-09-03 改名为 `Axwen/RAG`）并推送成功，CI 已于 2026-09-02 跑通——首轮四条工作流全红，三轮共抓到五个本地永不复现的真缺陷（全新克隆上 `bootstrap` 必失败、文档死链、audit job 缺 `.env`、单测依赖未构建的 `packages/*/dist`，外加配置错 `setup-uv@v10` 不存在），已全部修复，第三轮四条工作流全绿，留档于 [CI/CD 文档](docs/engineering/ci-cd.md) §6；CodeQL 与 dependency-review 因 private repo 缺 Code Security 一起改为按仓库变量 `ADVANCED_SECURITY_ENABLED` 开关，默认 skip。第四轮为全绿后的主动审计，九项改动含两个 P0（`check:commits` 十次运行检查了 0 条提交、`release.yml` 第一次推标签会死在缺 uv 上），见 §6.4。**剩余待用户动作（2026-09-03 更新）**：分支保护、Advanced Security、Dependabot PR 三项都已随仓库改名 + 转公开落地（见上面第六轮与 CI/CD 文档 §6.6）；本地 `git remote` 已由用户于 2026-09-03 改为 `git@github.com:Axwen/RAG.git`（旧名靠重定向也能 fetch/push，但每次 push 会多打一行 `This repository moved`，且写侧 `gh api` 对旧名返回 307 静默不生效）。两个 Dependabot major 都已了结：① typescript 6.0.3 的 `TS2591` 由 PR #19 预修（只给 `packages/contracts` 的 tsconfig 显式写 `types: ["node"]`，不动 base），#8 随后合入；② vite 8.2.2 的 `functions 80.76%` 不是覆盖率量法退化，而是 vite 8 换 Oxc/Rolldown 后不再消掉那个只在参数类型位置出现的 import，`apps/api/src/database/prisma.service.ts` 第一次被计入报告、暴露三个未测函数——补 `apps/api/test/prisma.service.test.ts` 后 functions 回到 83.65%，升级与单测一起合入，取代 #9（详见 CI/CD 文档 §6.6）。这两次都**不需要人工评论 `@dependabot rebase`**：`dependabot.yml` 没写 `rebase-strategy`，默认 `auto`，base 一动它自己 force push。真正待办只剩一件：③ `release.yml` 至今 0 次运行，第一次打 `v*` 标签会对外产出 GHCR 包与 GitHub Release，属需单独授权的动作，且 CHANGELOG 目前只有 `## [Unreleased]`、缺对应 `## [x.y.z]` 段落会先在 guard 的 `release-notes.sh` 里失败（GHCR 推送排在它之后，不会留下孤儿镜像）。/plan-devex-review boomerang 已于 2026-09-01 跑完（dx-baseline 脚本已落，两个目标达成，见 [复测报告](docs/engineering/plan-devex-review-20260901-boomerang.md)）；verify 门禁阈值暂不设定，待 CI 有真实分布后再定；devex T5/T7 仍未做，低优先级自由安排。后续每个实施批次都按 HG-02 至 HG-07 停顿并等待人工结论。
...
0. 读取仓库根的 [`AGENTS.md`](AGENTS.md)：事实源层级、六条 required checks 对应的本地
   命令、Prisma 与包边界约定、明确挂起不做的事。它不重复设计文档，只写「不知道就会返工
   或让 CI 变红」的部分。
