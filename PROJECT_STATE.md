...
当前状态：架构和工程协议已完成评审，六个架构探针的外部事实已完成（PROBE-001~004 为 `PASS`，PROBE-005~006 为 `PASS_WITH_ADJUSTMENT`）；T0 Monorepo 与本地开发基线已真实环境验收通过（2026-08-28）；**T1a Manifest/Prisma Core 切片 + devex P1 三项已于 2026-09-01 获用户 `ACCEPTED_WITH_ACTIONS`**（条件见[验收记录 §6](docs/engineering/acceptance/hg-01-t1a-manifest-core.md#6-用户验收结论)：租户谓词与请求体 `tenantId` 退场归 T14 DoD，幂等重放 201→200 归 T2）。HG-01 门禁本身尚未通过——完整范围还差 T12a(Ledger/配置骨架)、T11a(同步审计骨架)、T14a/T14b(身份与授权)（前置文档已于 2026-09-03 补齐：ADR-0040 与 T11 票据、T14 批次划分、HG-01a 临时门禁登记、`AGENTS.md` 与 T11/T12 的三份冻结契约，见[下一步执行顺序](#下一步执行顺序)第 4 条）。**T12a 已于 2026-09-03 开工，三片里的前两片（价目与配额配置、`model_budget_ledger` 与 `domain_audit_event` 两表及迁移）已落地并自测通过，五条事务入口与审计写入口未开始**，T14a/T14b 代码未开始，因此 T2/T10/T3/T1b 仍未解锁。除 T1a 的 Manifest/Release 领域面外尚无业务实现；容器级集成测试（Testcontainers）、Playwright、部署和真实语料性能基线均未验证。探针结果索引、Probe Decision Gate 和 ADR-0037 记录了已冻结事实、集成测试门槛与生产治理遗留项。
...
- T0 工程骨架与 T1a 切片（Manifest/Release 领域模型、内容寻址、兼容矩阵、领域命令端点）已提交于 `bc99b0a`；同一批次的 T14 身份与授权未开始；T12a 已落配置切片（价目、汇率与五项配额的启动期 schema）与两张表（`model_budget_ledger`、`domain_audit_event`，同一次迁移），五条事务入口与 `writeAuditEvent` 未开始，因此**运行时仍然没有任何东西在预扣、结算、拒绝或写审计**；T11a 的表随这次迁移一起落了，它的原因码注册表与写入口仍未开始。`tenantId` 目前由请求体携带，只能在本地开发环境使用。T14 的计划已按 [ADR-0039](docs/adr/0039-business-identity-and-unified-authorization.md) 明确为“Keycloak/OIDC 身份 + 自有 BusinessUser/租户/Workspace/角色/能力权限 + 资源 ACL”，后续客服、研发、普通员工三个角色工作台复用该身份上下文，不复制用户体系。T1b 分块、Release 状态迁移（`BUILDING` 及之后属 T5）、消息、检索、回答和 UI 仍未开始。
