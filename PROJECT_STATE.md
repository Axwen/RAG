...
- 已完成 **T14b 统一授权入口与资源策略**（2026-09-08，待评审）：`@rag/contracts` 授权契约（`AuthorizationDecision` 三类拒绝原因、`acl_scope_key` 定形 `t:{tenantId}:ks:{ksId}`、authz.* 审计码四枚入注册表）；`tenants.aclRevision` 列与 `bumpAclRevision`（成员变更必须同事务递增，ADR-0026）；`packages/database` 授权查库四入口（能力解析两层并集、作用域编译集合与 revision 同次读取、候选复核批量减法）；`apps/api` 全局 `AuthorizationService`（能力先于资源、每次判定写同步领域审计、允许路径审计写失败不放行、依赖不可用 fail closed）；**T14 DoD 两条退场已执行**——manifests 五 schema 移除请求体 `tenantId`（测试钉住「携带不生效」）、`approve*`/`GET /releases/:id` 全部带租户谓词（跨租户 id → 404 NOT_FOUND 不是 403，测试钉住）；IdentityGuard 从会话推导单一 ACTIVE 租户（多个 fail closed，多租户切换器归 T16）。smoke-api.sh 改走完整 OIDC（`/auth/login` → 表单登录 → 回调带完整 query 含 state）。
