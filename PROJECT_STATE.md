...
补充状态（2026-09-08）：T14b 本轮仅闭合统一授权接线、活动租户选择和 Workspace→KnowledgeSpace 作用域绑定；候选复核的删除墓碑、Legal Hold、有效期及复核超时仍分别等待 T5/T8/T6，故 HG-01 继续保持未通过。
...
- **T14b 统一授权入口与资源策略部分完成**（2026-09-08；代码评审后修复）：`@rag/contracts` 授权契约、`tenants.aclRevision`/`bumpAclRevision`、数据库授权查库入口和全局 `AuthorizationService` 已落地；Manifest/Release 端点已在领域调用前统一校验 `document.review`/`release.approve`，拒绝映射为统一错误信封；领域审计写失败按 ADR-0040 向上失败；IdentityGuard 支持签名会话中的活动租户，新增 `/auth/tenants/:tenantId/select` 刷新业务身份并保留原会话过期时间；新增 `WorkspaceKnowledgeSpace` 租户级复合外键，指定 Workspace 时作用域仅包含绑定知识空间。**尚未闭合**：候选复核的删除墓碑、Legal Hold、有效期字段和状态机等待 T5/T8 模型；复核超时接线等待 T6；多角色浏览器 E2E 等待 T16/HG-01。不得宣称 T14b 或 HG-01 已完成。
