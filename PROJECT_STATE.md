...
- 已完成 **T14a 身份接入与业务身份模型**（2026-09-07）：ADR-0039 的 7 张业务身份表及独立迁移（`20260907063419_t14a_business_identity`）、`packages/database` 的 `loadIdentityContext` 装配入口（判别联合区分未建档/禁用，不 JIT、不过滤成员状态）、`packages/contracts/src/auth` 的服务端身份上下文与 SessionView 投影（不外发 issuer/subject）、`apps/api/src/auth` 的 OIDC 授权码 + PKCE + JWKS 会话（`/auth/login|callback|session|logout`；会话过期 401、Keycloak 不可用 503、`AUTH_REQUEST_TIMEOUT_MS` 防网络黑洞）、开发种子（三 Workspace 多角色）与 `init-keycloak.sh` 的 partialImport 固定 UUID 用户 + `rag-api` 客户端。Keycloak 七类场景（PKCE、JWKS 轮换、过期、禁用、撤权、不可用、恢复）分两层：五类真实容器集成（`tests/keycloak-oidc.test.ts`，本地跑）、两类单元（本地桩）。**HG-01a 人工验收待执行**：登录链路尚未在真实浏览器走通（登录页归 T16 Design Review），授权判定与 `tenantId` 退场归 T14b。
