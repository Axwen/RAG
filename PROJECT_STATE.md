...
当前状态：架构和工程协议已完成评审，六个架构探针的外部事实已完成（PROBE-001~004 为 `PASS`，PROBE-005~006 为 `PASS_WITH_ADJUSTMENT`）；T0 Monorepo 与本地开发基线已真实环境验收通过（2026-08-28）；**T1a Manifest/Prisma Core 切片 + devex P1 三项已于 2026-09-01 获用户 `ACCEPTED_WITH_ACTIONS`**（条件见[验收记录 §6](docs/engineering/acceptance/hg-01-t1a-manifest-core.md#6-用户验收结论)：租户谓词与请求体 `tenantId` 退场归 T14 DoD，幂等重放 201→200 归 T2）。HG-01 门禁本身尚未通过——完整范围还差 T12a(Ledger/配置骨架)、T11a(同步审计骨架)、T14a/T14b(身份与授权)，代码均未开始（前置文档已于 2026-09-03 补齐：ADR-0040 与 T11 票据、T14 批次划分、HG-01a 临时门禁登记），因此 T2/T10/T3/T1b 仍未解锁。除 T1a 的 Manifest/Release 领域面外尚无业务实现；容器级集成测试（Testcontainers）、Playwright、部署和真实语料性能基线均未验证。探针结果索引、Probe Decision Gate 和 ADR-0037 记录了已冻结事实、集成测试门槛与生产治理遗留项。
