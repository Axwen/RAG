...
当前状态：架构和工程协议已完成评审，六个架构探针的外部事实已完成（PROBE-001~004 为 `PASS`，PROBE-005~006 为 `PASS_WITH_ADJUSTMENT`）；T0 Monorepo 与本地开发基线已真实环境验收通过（2026-08-28）；**T1a Manifest/Prisma Core 切片 + devex P1 三项已于 2026-09-01 获用户 `ACCEPTED_WITH_ACTIONS`**（条件见[验收记录 §6](docs/engineering/acceptance/hg-01-t1a-manifest-core.md#6-用户验收结论)：租户谓词与请求体 `tenantId` 退场归 T14 DoD，幂等重放 201→200 归 T2）。**T12a、T11a、T14a/T14b 已完成，HG-01 于 2026-09-09 获用户 `ACCEPTED`**（[HG-01 验收记录](docs/engineering/acceptance/hg-01-t14b-authorization.md)）：T14b 的统一授权入口、资源策略、活动租户选择、租户谓词、Workspace 作用域绑定、数据等级拒绝、同步授权审计和 fail-closed 行为已通过用户 API/数据库核验及自动化佐证。T2/T10/T3/T1b 已解锁。除 T1a 的 Manifest/Release 领域面外尚无业务实现；容器级集成测试（Testcontainers）、Playwright、部署和真实语料性能基线均未验证。探针结果索引、Probe Decision Gate 和 ADR-0037 记录了已冻结事实、集成测试门槛与生产治理遗留项。

补充状态（2026-09-09）：用户已完成 T14b/HG-01 人工核验并明确给出 `ACCEPTED`。本轮仅闭合统一授权接线、活动租户选择和 Workspace→KnowledgeSpace 作用域绑定；候选复核的删除墓碑、Legal Hold、有效期及复核超时仍分别等待 T5/T8/T6，不影响本次 HG-01 门禁结论。
...
- 当前 HG-01 批次已由用户于 2026-09-09 明确 `ACCEPTED`；下一批次按门禁表解锁，但完成后仍须再次暂停并等待 HG-02 人工结论。
...
- **T14b 统一授权入口与资源策略已通过 HG-01 人工验收**（2026-09-08 实现，2026-09-09 `ACCEPTED`；代码评审后修复）：`@rag/contracts` 授权契约、`tenants.aclRevision`/`bumpAclRevision`、数据库授权查库入口和全局 `AuthorizationService` 已落地；Manifest/Release 端点已在领域调用前统一校验 `document.review`/`release.approve`，拒绝映射为统一错误信封；领域审计写失败按 ADR-0040 向上失败；IdentityGuard 支持签名会话中的活动租户，新增 `/auth/tenants/:tenantId/select` 刷新业务身份并保留原会话过期时间；新增 `WorkspaceKnowledgeSpace` 租户级复合外键，指定 Workspace 时作用域仅包含绑定知识空间。**不在本批次范围**：候选复核的删除墓碑、Legal Hold、有效期字段和状态机等待 T5/T8 模型；复核超时接线等待 T6；多角色浏览器 E2E 与用户主链 UI 等待 T16。详见 [HG-01 验收记录](docs/engineering/acceptance/hg-01-t14b-authorization.md)。
...
- T0 工程骨架与 T1a 切片（Manifest/Release 领域模型、内容寻址、兼容矩阵、领域命令端点）已提交于 `bc99b0a`；T12a/T11a/T14a/T14b 已分别落地并纳入 HG-01：配置与预算账本事务入口、同步审计写入口、业务身份模型、统一授权入口和资源策略均已有代码与测试，HG-01 已于 2026-09-09 获用户 `ACCEPTED`。模型调用侧（供应商方言、`usage.cost` 读取、429 退避、流式取消触发结算）仍归 T15；T1b 分块、Release 状态迁移（`BUILDING` 及之后属 T5）、消息、检索、回答和 UI 仍未开始。`tenantId` 已从 Manifest/Release 请求体退场，租户上下文由已验证身份推导；T2/T10/T3/T1b 已解锁但等待用户授权后再启动。
...
- 每日 16 元、用户级配额和 24 至 36 周窗口仍需在真实链路下校准；票据级工作量估算已补齐，但周期换算系数尚未用真实交付速率验证。完整 24 GiB Parser 余量也尚未验证。ADR-0034 的五个配额值与 PROBE-005 反算的单价、汇率 7.2 已于 2026-09-03 以启动期配置默认值落地（`packages/config/src/resource-limits.ts`、`packages/config/src/model-pricing.ts`）；T12a 的预算账本预扣/结算/释放/lease 回收入口已完成，T12b 的用户级限流、缓存失效和性能报告仍未开始。
...
**状态勘误（2026-09-09）**：上段“前置待用户动作”记录的是 2026-09-03 的历史审计过程，不再代表当前待办。当前唯一新增的 CI 处理项是 PR #38 合并后 Dependabot 任务 `34306363546` 暴露的 `multer` 安全更新：修复分支已将 `@nestjs/platform-express@12.0.1` 的传递依赖从 `2.2.0` 提升到修复版 `2.3.0`，待 PR 检查完成后关闭。
