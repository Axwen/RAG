...
- HTTP 200，耗时 2.273s，内容 53 字符
- 响应 id：True；request-id 头：None
- model 回显：`step-3.5-flash-2603`（与请求一致：True）
- finish_reason：`stop`；请求 max_tokens=1200，被截断：False
- **隐藏推理正文**：`reasoning_content` 字段存在=True，356 字符（与可见正文共用同一 max_tokens 预算）
- usage 回传：**True** → {'prompt_tokens': 165, 'completion_tokens': 280, 'total_tokens': 445, 'reasoning_tokens': 0, 'cached_tokens': 64}
- 回答含资料编号引用：True
...
|---|---|---|---|---|---|---|---|---|
| 完整生成 | 200 | 0.305 | 1.918 | 2.078 | 11/58 | 79/288 | stop | 有 |
| 中途取消(5 增量) | 200 | 0.29 | 1.776 | 1.879 | 5/21 | 76/290 | None | 有 |
...
|---|---|---|---|---|---|---|---|---|---|
| `json_schema` | 200 | [200] | True | True | stop | False | 155/301 | 1200 |  |
| `json_object` | 200 | [200] | True | True | stop | False | 130/687 | 1200 |  |
...
|---|---|---|---|---|---|
| `(unset)` | [200, 200, 200] | 286/337/386 | [157, 184, 207] | [0, 0, 0] | ['stop', 'stop', 'stop'] |
| `low` | [200, 200, 200] | 45/45/49 | [29, 29, 31] | [0, 0, 0] | ['stop', 'stop', 'stop'] |
| `high` | [200, 200, 200] | 381/416/440 | [206, 240, 231] | [0, 0, 0] | ['stop', 'stop', 'stop'] |

- 返回推理正文：**True**；usage 中单列 reasoning_tokens：**False**
- `reasoning_effort` 被接受：True；**区间可分离（判定为有效）：True**（各档 CoT 字数 min-max {'(unset)': [286, 386], 'low': [45, 49], 'high': [381, 440]}）
...
- 样本 5/5 成功；完整生成 **2.049–3.236s**（中位 2.39s）；可读答案首字 1.902–2.647s（中位 2.187s）
- 越界次数：>3.5s（高风险预算）**0** 次；>2.0s（常规预算）**5** 次
- 口径：同一 payload 的流式完整生成，n=5；ADR-0027 以 P95 表述，n 太小只能给区间与越界次数，不能自称 P95。

| # | HTTP | 完整生成(s) | 首事件(s) | 可读答案(s) | 正文字符 | 推理字符 | completion_tokens |
|---|---|---|---|---|---|---|---|
| 1 | 200 | 2.049 | 0.331 | 1.902 | 57 | 288 | 237 |
| 2 | 200 | 2.307 | 0.302 | 2.187 | 53 | 354 | 280 |
| 3 | 200 | 3.236 | 0.388 | 2.647 | 53 | 354 | 280 |
| 4 | 200 | 2.42 | 0.479 | 2.165 | 69 | 297 | 247 |
| 5 | 200 | 2.39 | 0.353 | 2.243 | 53 | 336 | 268 |
...
- **单次问答**（一次 grounded chat，含隐藏 CoT）：prompt 165 + completion 280 tokens → None 元
- 本次探针合计（下界）：prompt tokens 1320；completion tokens 3355
...
- 问询用 max_tokens=512，finish_reason=`stop`，CoT 405 字符；因截断而取不到身份：False
...
## 待决策
- 高风险 3.5s 预算本轮全部满足，但常规 2.0s 预算有 5/5 次越界（完整生成 n=5：2.049–3.236s（中位 2.39s）；可读答案首字 1.902–2.647s）：常规问答腿需按上界而非中位数设超时，且逐句 Embedding 与蕴含调用必须并发发起（ADR-0027 硬约束）。历史运行曾达 7.7-9.7s，抖动须计入选型。
- **推理内容先流完才出正文**：首个任意事件 0.305s、可读答案首字 1.918s（差 1.61s，其间只有 79 个 reasoning_content 增量）。UI 只能先显示「思考中」进度，不能把 reasoning_content 当答案渲染（未经引用校验的内容不得呈现，ADR-0027/0032）。
- **推理用量不可分摊**：`message.reasoning_content` 实测数百字，但 `usage.completion_tokens_details.reasoning_tokens` 恒为 0 —— CoT 计费折进 `completion_tokens` 却不单列。ADR-0029 账本因此无法把成本拆成「思考 vs 答案」，只能按 completion_tokens 总量结算；且 Adapter 必须把 CoT 计入 max_tokens 预留，否则正文被静默截断（实测 CoT 字数 {'(unset)': [286, 386], 'low': [45, 49], 'high': [381, 440]}）。
- `reasoning_effort` **实测有效**（各档 CoT 字数 min-max：{'(unset)': [286, 386], 'low': [45, 49], 'high': [381, 440]}，每档 3 次采样，low 上界与 high 下界分离）：可作为高风险腿压时延的候选手段，但需与「低 effort 是否降低蕴含判断质量」一并在 PROBE-006 评测，**未评测前不得为了达标 ADR-0027 而默认调低**。
