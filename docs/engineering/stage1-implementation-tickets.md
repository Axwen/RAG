...
>
> 当前执行位置（2026-09-09）：T12a、T11a、T14a、T14b 已完成，HG-01 已获用户 `ACCEPTED`；
> T2/T10/T3/T1b 已解锁但尚未启动，等待用户明确下一批次。
...
| [T11](tickets/T11-audit-telemetry.md) Audit/Telemetry | 同步领域审计与异步运行遥测：两条载体在包依赖图上互相看不见 | **T11a 已完成并纳入 HG-01**（契约、`domain_audit_event`、同事务写入口、T12a/T14b 接入）；T11b 异步遥测与恢复依赖 T3 的 Outbox，留在 HG-02 之后；其余接入面随各域票据推进（ADR-0040） |
| [T12](tickets/T12-performance-budget.md) Performance/Budget | 配置硬上限、Budget Ledger、限流、缓存和性能报告 | **T12a 已完成并纳入 HG-01**（Ledger schema、预扣/结算/释放/回收入口、配置骨架）；T12b 限流缓存与性能报告未开始，检索性能随 T6、完整报告随 T9 后关闭 |
| T13 Untrusted Content | 解析、进入上下文前、输出后三处注入检测 | 分别随 T4、T6、T7 交付，不作为最后一次集中补丁 |
| [T14](tickets/T14-identity-authorization.md) Identity/Business User/Authorization | Keycloak OIDC 只负责身份；自有 BusinessUser、租户/Workspace 成员、角色/能力权限、资源授权、作用域编译和 fail-closed | **T14a/T14b 已完成**；T14a 已通过 HG-01a，T14b 已随 HG-01 于 2026-09-09 获用户 `ACCEPTED`。T6 和后续 Web 路由仍需按各自票据接入统一身份上下文，不复制用户系统（ADR-0039）|
...
探针收尾提交
  ...
  -> T14b(统一授权入口)
  -> HG-01 人工核验（2026-09-09 ACCEPTED；当前停在此处，等待用户授权下一批次）
  -> T2 + T10(Worker 基础) -> T3 -> T1b（已解锁，未启动）
  -> HG-02 人工核验
