...
- **T14a 身份接入与业务身份模型（已完成）** — Keycloak OIDC 接入（Authorization Code + PKCE、JWKS 校验与轮换、会话过期、身份服务不可用映射）、`(issuer, subject)` 到 `BusinessUser` 的唯一映射、7 张业务身份表及其迁移与开发种子、服务端身份上下文契约已落地，并于 2026-09-08 通过 HG-01a 人工验收。human: ~4.75d / CC: ~1.2d。
- **T14b 统一授权入口与资源策略（已完成，纳入 HG-01）** — 能力权限判定、资源策略、数据等级拒绝、`acl_scope_key` 允许作用域编译、批量候选权威复核、任何依赖不可用即 fail closed、撤权竞态与复核超时边界已按当前批次落地，并于 2026-09-09 随 HG-01 获用户 `ACCEPTED`。human: ~3.25d / CC: ~0.8d。

两批合计仍按 ~8d / ~2d 记，不因拆分改动数字。

T14a 结束时增设的临时人工门禁 [HG-01a](../manual-acceptance-gate.md#阶段-1-门禁点) 已于 2026-09-08 通过；随后 T14b 的授权判定与受保护路由完成，并在 HG-01 于 2026-09-09 获用户 `ACCEPTED`。

T14b 已在 [T11a](T11-audit-telemetry.md#批次划分) 收口后实现：授权决策写同步领域审计，原因码使用 T11a 的中央注册表（[ADR-0040](../../adr/0040-domain-audit-and-runtime-telemetry.md) 决策 3）。
