...
索引文档不再携带授权主体和 ACL 版本。文档级和 Chunk 级投影只携带随文档版本本身确定、成员变更不会改变的稳定作用域字段：`tenant_id`、`knowledge_space_id`、`data_class`、`visibility_class`，并由它们拼成 `acl_scope_key`。这些字段只在文档自身的分级或所属知识空间发生变化时改变，而那本身就是新的文档版本或新的投影，属于正常的 Release 构建路径。
