...
- **2026-09-06（ADR-0042）**：Embedding 维度从全局常量语义修订为 `EmbeddingChannel` 的显式身份；文档 1024 维保留为 `DOCUMENT_EMBEDDING_DIMENSION` 兼容基线，不再由 `@rag/rag-core` 导出为全局 `EMBEDDING_DIMENSIONS`。
