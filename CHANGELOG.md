...
- **T14b 授权接线与多租户修复**：Manifest/Release 所有创建、批准和读取端点统一经过
  `AuthorizationService`；领域审计写失败不再被拒绝路径吞掉；会话签名携带活动租户并提供
  租户选择端点；新增 `WorkspaceKnowledgeSpace` 复合租户外键与作用域编译裁剪。候选复核的
  删除墓碑、Legal Hold、有效期仍待 T5/T8 模型落地后补齐。
