...
if command -v node >/dev/null 2>&1; then
  ...
  if [[ "${node_version}" == "${want_node}" ]]; then
...
# ── uv（Python 工具链，仅 Parser/评测需要）─────────────────────────────
if command -v uv >/dev/null 2>&1; then
  have "uv $(uv --version 2>/dev/null | awk '{print $2}')"
else
  need "uv 未安装：curl -LsSf https://astral.sh/uv/install.sh | sh（Parser 与评测需要；只跑 API/Web 可跳过）"
  fail=1
fi
