...
#
# 不装依赖、不需要 node：只读 lockfile + HTTPS POST，python3 标准库就够，job 从 15 分钟回到秒级。
...
def parse_args(argv):
    ...
    usage = '可用参数：--audit-level --lockfile --registry --rounds'
    it = iter(argv)
    for arg in it:
...
def post_json(url, payload, rounds):
    """一次 bulk 查询。网络类错误重试；取不到数据一律 fail closed，不降级成「没有漏洞」。"""
    ...
    for attempt in range(1, rounds + 1):
        ...
        try:
            ...
            if exc.code not in RETRYABLE:
                try:
                    ...
                    detail = '(读不到响应体)'
                die(NO_DATA, f'❌ bulk 端点返回 {exc.code}：{detail}')
        ...
        if attempt < rounds:
            ...
            time.sleep(wait)
    die(NO_DATA, f'❌ bulk 端点 {rounds} 轮均取不到数据（最后一次 {last}）：'
                 '这不是「没有漏洞」，是取不到数据')
...
def collect(url, wanted, rounds):
    names = sorted(wanted)
    found = {}
    for start in range(0, len(names), CHUNK):
        chunk = {name: wanted[name] for name in names[start:start + CHUNK]}
        for name, advisories in post_json(url, chunk, rounds).items():
    ...
    for name in sorted(found):
        ...
        if len(versions) > 1:
            ...
            for version in versions:
                for advisory in post_json(url, {name: [version]}, rounds).get(name, []):
...
def main(argv=None):
    ...
    rows = collect(url, wanted, rounds)
